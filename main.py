...
class User(Base):
  ...
  def __repr__(self):
    return "<User(idUser={}, idGuild={}, nome={}, pdl={})".format(
      self.idUser, self.IdGuild, self.nome, self.pdl)
...
class Partida(Base):
  ...
  def __repr__(self):
    return "<User(idPartida={}, time1={},time2={}, ganhador={})".format(
      self.idPartida, self.time1, self.time2, self.vencedor)
...
async def go(ctx):
  ...
  user = session.query(User).filter_by(idUser=user_id,
                                       IdGuild=guild_id).first()
...
async def leave(ctx):
  ...
  if user and user.idUser in [
      member.idUser for member in guild_queues.get(guild_id, [])
  ]:
    guild_queues[guild_id] = [
      member for member in guild_queues[guild_id]
...
async def startPartida(players, ctx):
  ...
  partida = Partida(time1, time2, guild_id)
...
async def rank(ctx):
  ...
  guild_id = ctx.guild.id
  users = session.query(User).filter_by(IdGuild=guild_id).order_by(
    User.pdl.desc()).all()
...
async def end(ctx, id_partida: int, time_vencedor: str):
  ...
  try:
    ...
    partida = session.query(Partida).filter_by(idPartida=id_partida,
                                               idGuild=guild_id).first()
    ...
    session.commit()
    await ctx.send(
      f"Partida com ID {id_partida} encerrada. Time vencedor: {time_vencedor}")
  ...
  equipe_vencedora = partida.get_time1(
  ) if time_vencedor == 'time1' else partida.get_time2()
  equipe_perdedora = partida.get_time2(
  ) if time_vencedor == 'time1' else partida.get_time1()

  vencedores = []

  for user in equipe_vencedora:
    userAtt = session.query(User).filter_by(idUser=user.idUser,
                                            IdGuild=ctx.guild.id).first()
    userAtt.win += 1
    userAtt.pdl += 7
